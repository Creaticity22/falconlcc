...
import { BookOpen, Star, CheckCircle2, PlayCircle, RotateCcw } from "lucide-react";
...
export default function LessonCard({ lesson, completed, isNext, score }: LessonCardProps) {
        ...
        <div className="flex-1 min-w-0">
          ...
          {completed && (
            <div className="flex items-center flex-wrap gap-2 mt-2.5">
              ...
              <span className="ml-auto inline-flex items-center gap-1 text-[10px] font-bold text-primary bg-primary/5 hover:bg-primary/10 border border-primary/30 px-2.5 py-1 rounded-full">
  ...
  return <Link to={`/learn/${lesson.id}`}>{cardInner}</Link>;
}
